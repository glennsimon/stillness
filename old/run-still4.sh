...
do
  ...
  then
    INPUT=""
    NOW=$( date '+%H:%M:%S' )
    get_temp "28-032197797f0c"
    AMBTEMP=$TEMP
    get_temp "28-032197792401"
    BOILERTEMP=$TEMP
    get_temp "28-032197794fef"
    STILLHEADTEMP=$TEMP
    get_temp "28-0321977926b2"
    COOLINTEMP=$TEMP
    get_temp "28-032197797070"
    COOLOUTTEMP=$TEMP
    DATA_ROW=($NOW $PWM $AMBTEMP $BOILERTEMP $STILLHEADTEMP $COOLINTEMP $COOLOUTTEMP $DESIREDSTILLHEADTEMP $JAR $DISTILLATEVOL)
    clear
    reset_display
    i=0
