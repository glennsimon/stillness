...
instr5="4 - quit"
headers1=("" "" "T boil" "T boil" "jar" "amt coll")
headers2=("Time" "PWM" "(C)" "tgt(C)" "#" "(ml)")
horizLine="---------------------------------------------------------"
...
for ((i=0; i < ${#headers1[@]}; i++))
do
  printf "%${spacings[i]}s |" "${headers1[i]}" >> $dataFilename
done
echo "" >> $dataFilename
for ((i=0; i < ${#headers2[@]}; i++))
do
  printf "%${spacings[$i]}s |" "${headers2[$i]}" >> $dataFilename
done
echo "" >> $dataFilename
...
reset_display()
{
  ...
  for ((i=0; i < ${#headers1[@]}; i++))
  do
    printf "%${spacings[$i]}s |" "${headers1[$i]}"
  done
  echo ""
  for ((i=0; i < ${#headers2[@]}; i++))
  do
    printf "%${spacings[$i]}s |" "${headers2[$i]}"
  done
  echo ""
  echo $horizLine
  for ((i=0; i < ${#dataRow[@]}; i++))
  do
    printf "%${spacings[$i]}s |" "${dataRow[$i]}"
  done
  echo ""
  echo "PWM = " $PWM
  echo "stepChange = " $stepChange
  echo "diff = " $diff
  echo "N = " $count "; mPWM = " $mPWM "; KPWMAVG = " $mPWMAvg "; integral = " $integral
}
...
do
  ...
  then
    ...
    dataRow=($NOW $PWM $boilerTemp $boilerTarget $jar $distillateVol)
    clear
    reset_display
    i=0
